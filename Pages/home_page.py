...
def second_header(st):
    ...
    second_header_util(left, icon="", heading="Easy and Quick Appointment Scheduling", content="Book a non-clashing appointment with a doctor of your choice, in just a few simple steps. Visit out 'Book An Appointment' page to know more")
    second_header_util(middle, icon="", heading="Availability of Doctor based on Location", content="You can find the doctors based on their location, and make yourself comfortable with whichever location suits you best")
    second_header_util(right, icon="", heading="Sort the Doctors based on your needs", content="Not just locations, we provide a variety of variables for you to select whichever factor you need the most, wether it be fees or experience, we value your choice")

    Utils.add_space(st)

    left, middle, right = st.empty().columns(3)

    second_header_util(left, icon="", heading="Enjoy seemless Real Time Scheduling", content="Enjoy seemless easy-to-use real time booking of your appointment. No need to worry about other ends, we take care of it")
    second_header_util(middle, icon="", heading="Have any Problem? worry not we have FAQs Section", content="If you feel stuck or have any problem, just visit our FAQs section for answers to some of our most common questions... Still Stuck? Visit our contact us page and reach out to us")
    second_header_util(right, icon="", heading="Use our easy to view Maps for location understandings", content="We also provide an easy-to-view instant maps for all our location services. This helps you as a user to have a quick view of the surrounding terrains of your appointment destination")
